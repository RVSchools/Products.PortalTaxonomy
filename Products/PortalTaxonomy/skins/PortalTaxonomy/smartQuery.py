...
##parameters=REQUEST=None,show_all=0,quote_logic=0,quote_logic_indexes=['SearchableText','Description','Title'],use_types_blacklist=False,show_inactive=False,use_navigation_root=True
...
#--------------------------------------------------------------
# OBJECTIVES
# Convert to use AND all the time
# Strip any unused stuff
#--------------------------------------------------------------
...
for k in REQUEST.keys():
    ...
    if k=='categories':
        k = 'getCategories'
        cats = [l for l in v if l]
        if cats:
            v = {'query': cats, 'operator': 'and'}
        else:
            v = None
        #query[k] = v
...
if show_query:
    try:
        ...
        query['path'] = '%s%s' % (portal_path, REQUEST.search_path)
